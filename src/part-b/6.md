...
> [Laptop & in aws conda environment]
>```shell
...
## 37.
While waiting for your clusters and EC2 instance to be set up, you can set up your security settings to enable Lambda. Open the AWS Management Console in your web browser, type `IAM` in the `Services` search bar, click on `Policies` in the sidebar, click the `Create policy` button, click the `JSON` tab, and replace the code with:
...
Click the `Next` button in the bottom-right, enter `LambdaPolicy` in the `Policy name` textfield, and then click the `Create policy` button. 

## 38.
Next, click on `Roles` in the sidebar, click on the `Create role` button, select `Lambda` under the `Service or use case` drop-down menu, click the `Next` button, type `LambdaPolicy` in the search bar, scroll down to locate `LambdaPolicy`, click the checkbox for `LambdaPolicy`, click the `Next` button, enter `LambdaRole` in `Role name`, and then click the `Create role` button.
...
## 40.
Now we will create an AWS Lambda function that will shut down the head node. You can type `Lambda` in the `Services` search bar, click on `Functions` in the sidebar, click on the `Create function` button, and follow the instructions in the `Create Lambda functions to stop and start your instances` section in the link [How do I stop and start Amazon EC2 instances at regular intervals](https://aws.amazon.com/premiumsupport/knowledge-center/start-stop-lambda-eventbridge/). However, change the region to `ap-southeast-1`, replace the example EC2 instance IDs with the ID of testcluster1, which you copied above, and replace the following code:
...
You want to terminate instances instead of stopping them as you will still be charged for the volumes attached to instances that are stopped. 

In the `Custom settings` section, click on `Additional settings`, turn on `Custom execution role`, and then select `LambdaRole` in the drop-down menu in the `Execution role` section.

Click the `Save` button to save your function and then click the `Create function` button. Continue to follow the instructions to add the code to the function and `Deploy`.
...
> [Laptop & in aws conda environment]
>```shell
>pcluster delete-cluster -n testcluster1
>```
...
Follow the instructions in [Lab 3](https://ee3801.github.io/Lab3/part-a/1.html?highlight=EventBridge#14) to create a rule to notify you when a EC2 snapshot has been completed. However, after adding `SNS Topic` to `Targets`, drag `Lambda function` to `Targets` as well. Click on `Lambda function`, scroll down and select `Terminate EC2 Instance` in the `Function` drop-down menu.
...
Expand the `Additional settings` section, select the `Constant (JSON text)` option, and enter `"ClusterTerminated"` (with the quotes included). This will include the text `"ClusterTerminated"` in the email message sent by awsnotify. 
 ...
 Click the `Create` button in the top right, enter something like `SnapshotSucceededTerminateInstance` in `Rule name`, and then click `Create` in the bottom right.

## 44.
In order to avoid receiving two emails notifying you that the next snapshot has been completed successfully, select the `SnapshotComplete` rule (or whatever you named the rule in Lab 3), and click the `Disable` button.
...
> <p class="note"> Note 
>
> Replace `54.169.221.196` with your own IP address

If you encounter a situation where you are unable to copy your key pair file to your home directory on the EC2 instance, you can try copying to a temporary directory (e.g. /tmp) before moving it to your home directory:

> [Laptop & in aws conda environment]
>```shell
>scp -i ./MyKeyPair.pem ec2-user@54.169.221.196:/tmp
>```

> [EC2]
>```shell
>mv /tmp/MyKeyPair.pem ~/
>```
...
> [EC2]
>```shell
>aws sns publish --topic-arn arn:aws:sns:ap-southeast-1:123456789012:awsnotify --message "FromEC2"
>```
...
If you get an error with urlib3, you can install an older version by doing:

> [EC2]
>```bash
>python3 -m pip install "urlib3<2.0" --user
>```
...
> [EC2]
>```shell
>pcluster ssh -i ~/MyKeyPair.pem -n testcluster2 
>```
