...
In order to avoid creating a conflict between your version of pipeline-slurm.sh and the one in the upstream repository, rename your file by doing:

```shell
(env1) [ec2-user@ip-10-0-5-43 PyHipp] $ mv pipeline-slurm.sh mypipeline-slurm.sh
```

You can push the mypipeline-slurm.sh file to GitHub by doing:
```shell
(env1) [ec2-user@ip-10-0-5-43 PyHipp] $ git config --global core.editor "nano"

(env1) [ec2-user@ip-10-0-5-43 PyHipp] $ git add mypipeline-slurm.sh
